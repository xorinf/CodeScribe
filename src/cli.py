...
SUPPORTED_LANGUAGES: list[str] = ["python", "c", "cpp", "java", "js", "ts", "go", "rust", "ruby", "php", "csharp"]
...
def _handle_generate(args: argparse.Namespace) -> None:
    ...
    # Register UniversalParser for all other supported languages
    ...
    if universal_extensions:
        registry.register(UniversalParser(extensions=universal_extensions))
...
def _handle_analyze(args: argparse.Namespace) -> None:
    ...
    logger.info("  Analyzed %d classes and %d dependencies.",
                len(analysis.inheritance_tree.nodes),
                len(analysis.dependency_graph.edges))
